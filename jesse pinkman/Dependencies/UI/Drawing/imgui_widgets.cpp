...
bool ImGui::ButtonEx(const char* label, const ImVec2& size_arg, ImGuiButtonFlags flags)
{
    ...
    // Increase the button width by adding extra width (e.g., 50.0f) to the calculated width
    ImVec2 size = CalcItemSize(size_arg, label_size.x + style.FramePadding.x * 2.0f + 50.0f, label_size.y + style.FramePadding.y * 5.0f);
...
bool ImGui::Checkbox(const char* label, bool* v)
{
    ...
    const ImRect check_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(label_size.y + style.FramePadding.y * 1.f, label_size.y + style.FramePadding.y * 1.f)); // We want a square shape so we use Y twice
    ...
    ImU32 topColor = GetColorU32(ImGuiCol_Header);
    ImU32 bottomColor = topColor;

    if (*v) {
        // Checked state
        ImU32 checkedTopColor = GetColorU32(ImVec4(0.0824f, 0.6f, 0.3412f, 1.f));
        ImU32 checkedBottomColor = GetColorU32(ImVec4(0.0824f, 0.3412f, 0.6f, 1.f));
        ...
        // Unchecked state
        ImU32 gradientStartColor = GetColorU32(ImVec4(0.09f, 0.09f, 0.09f, 255));
        ImU32 gradientEndColor = GetColorU32(ImVec4(0.09f, 0.09f, 0.09f, 255));
        ImU32 topColorHovered = GetColorU32(ImVec4(0.09f, 0.09f, 0.09f, 255));
        ImU32 bottomColorHovered = GetColorU32(ImVec4(0.09f, 0.09f, 0.09f, 255));
        ...
        if (hovered) {
            ...
            window->DrawList->AddRectFilledMultiColor(check_bb.Min + ImVec2(3, 3), check_bb.Max - ImVec2(3, 3), gradientStartColor, gradientStartColor, gradientEndColor, gradientEndColor);
            window->DrawList->AddRect(check_bb.Min + ImVec2(3, 3), check_bb.Max - ImVec2(3, 3), borderColor, 0, false, 1);
        }
    }
...
bool ImGui::SliderScalar(const char* label, ImGuiDataType data_type, void* p_data, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    const ImVec2 label_size = CalcTextSize(label, NULL, true);
    const ImRect frame_bb(window->DC.CursorPos, window->DC.CursorPos + ImVec2(w, label_size.y + style.FramePadding.y * 0.1f)); // Adjust height to make slider smaller
    const ImRect total_bb(frame_bb.Min, frame_bb.Max + ImVec2(label_size.x > 0.0f ? style.ItemInnerSpacing.x + label_size.x : 0.0f, 0.0f));
    ...
    ImU32 bg_color = IM_COL32(25, 25, 25, 255); // RGB 25, 25, 25

    // Render the background of the slider
    window->DrawList->AddRectFilled(frame_bb.Min, frame_bb.Max, bg_color);

    // Define the gradient colors
    ImU32 color_start = IM_COL32(21, 153, 87, 255); // #ffd89b
    ImU32 color_end = IM_COL32(21, 87, 153, 255);    // #19547b

    // Calculate the fraction of the slider that is filled
    float grab_t = (float)((*(float*)p_data - *(const float*)p_min) / (*(const float*)p_max - *(const float*)p_min));

    // Render the filled gradient part of the slider
    ImVec2 gradient_start = frame_bb.Min;
    ImVec2 gradient_end = ImVec2(frame_bb.Min.x + grab_t * frame_bb.GetWidth(), frame_bb.Max.y);
    window->DrawList->AddRectFilledMultiColor(gradient_start, gradient_end, color_start, color_start, color_end, color_end);
    ...
    if (label_size.x > 0.0f)
        RenderText(ImVec2(frame_bb.Max.x + style.ItemInnerSpacing.x, frame_bb.Min.y + style.FramePadding.y - 2.0f), label); // Adjusted label position
