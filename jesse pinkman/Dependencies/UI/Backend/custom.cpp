...
//
//    float anim = ImTricks::Animations::FastFloatLerp(std::string(label).append("tab.anim"), selected, 0.f, 1.f, 0.17f);
...
//    line_pos = ImLerp(line_pos, selected ? bb.Min.y - window->Pos.y : line_pos, 0.13f);
...
//    ImVec2 p = window->DC.CursorPos;
//    //largeur des subtab
//    ImVec2 size({ CalcTextSize(label, 0, true).x + 50, window->Size.y });
...
//    bool pressed = ButtonBehavior(bb, id, &hovered, &held);
//
//    float anim = ImTricks::Animations::FastFloatLerp(std::string(label).append("subtab.anim"), selected, 0.f, 1.2f, 0.13f);
