...
void Game_::World_::get_bone_array( uintptr_t ent ) {
	...
	bone_ = device_( ).read <uintptr_t>( ent + 0x5B8 + ( device_().read <int>(ent + 0x600) * 0x10 ) );

	comp_ = device_( ).read<ftransform>(ent + 0x1c0 );
}
...
void Game_::Camera_::get_camera( ) {
	...
	fov = device_( ).read<float>( cache.get_pcontroller( ).value_or( 0 ) + 0x394 ) * 90.f;
