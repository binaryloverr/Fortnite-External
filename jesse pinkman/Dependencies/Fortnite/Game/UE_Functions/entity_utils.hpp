...
weapon_info_ Environment_::Weapons_::get_weapon_name( uint64_t actor ) {
	...
	uint64_t fname_text = device_( ).read<uint64_t>( weapon_data + 0x40 );
...
std::string Environment_::PState_::get_platform( uintptr_t ps ) {
	...
	uintptr_t test_platform = device_( ).read<uintptr_t>( ps + 0x438 );
...
bool Environment_::PState_::is_visible( uintptr_t skeletal_mesh ) {
	...
	auto ls = device_( ).read<float>( skeletal_mesh + 0x2e8 );
	auto lr = device_( ).read<float>( skeletal_mesh + 0x2f0 );
	return (bool)(lr + 0.06f >= ls);
}
...
std::string Environment_::PState_::get_player_name(uintptr_t player_state)
{
	...
	uintptr_t pNameStructure = device_( ).read<uintptr_t>( player_state + 0xab0 ); //pNameStructure
	if ( device_( ).IsValidAddress_( pNameStructure ) ) {
		...
		if ( device_( ).IsValidAddress_( pNameEncryptedBuffer ) ) {
			device_( ).read_array( pNameEncryptedBuffer, pNameBuffer, pNameLength );
...
void Environment_::drawing_::render_elements( )
{
	...
	outlined_text( ImGui::GetBackgroundDrawList( ), 5, 13, col1, "excision" );

	outlined_text( ImGui::GetBackgroundDrawList( ), 60, 13, col2, "slotted" );
